...
func Test_MultiUnderlayChannels(t *testing.T) {
	...
	underlay, err := dialer.CreateWithHeaders(time.Second, map[int32][]byte{
		TypeHeader:         []byte("default"),
		ConnectionIdHeader: []byte(uuid.New().String()),
	})
	req.NoError(err)
